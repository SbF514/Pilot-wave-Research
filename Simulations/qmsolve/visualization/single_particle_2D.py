...
class TimeVisualizationSingleParticle2D(TimeVisualization):
    ...
    def animate(self, xlim=None, ylim=None, figsize=(7, 7), animation_duration=5, fps=20, save_animation=False,
                potential_saturation=0.8, wavefunction_saturation=0.8):
        """
        ...
        L = self.simulation.H.extent / Å
        potential_plot = ax.imshow(
            (self.simulation.H.Vgrid + self.simulation.Vmin) / (self.simulation.Vmax - self.simulation.Vmin),
            vmax=1.0 / potential_saturation, vmin=0, cmap="gray", origin="lower", interpolation="bilinear",
            extent=[-L / 2, L / 2, -L / 2, L / 2])
        wavefunction_plot = ax.imshow(complex_to_rgba(self.simulation.Ψ_plot[0], max_val=wavefunction_saturation),
                                      origin="lower", interpolation="bilinear", extent=[-L / 2, L / 2, -L / 2, L / 2])

        # Initialize particles (dots)
        num_particles = 200
        # Get probability density from initial wavefunction
        prob_density = np.abs(self.simulation.Ψ_plot[0])**2
        
        # Apply Gaussian weight to favor center positions
        y, x = np.ogrid[:prob_density.shape[0], :prob_density.shape[1]]
        center_y, center_x = prob_density.shape[0] // 2, prob_density.shape[1] // 2
        gaussian_weight = np.exp(-((x - center_x)**2 + (y - center_y)**2) / (prob_density.shape[0]/4)**2)
        
        # Apply the weight to probability density
        prob_density = prob_density * gaussian_weight
        
        # Normalize to create proper probability distribution
        prob_density = prob_density / np.sum(prob_density)
        # Flatten to 1D for random choice
        prob_density_flat = prob_density.flatten()
        
        # Generate indices based on weighted probability distribution
        indices = np.random.choice(len(prob_density_flat), size=num_particles, p=prob_density_flat)
        # Convert back to 2D coordinates
        y_indices, x_indices = np.unravel_index(indices, prob_density.shape)
        
        # Convert indices to physical coordinates
        x_particles = (x_indices * L / self.simulation.H.N) - L/2
        y_particles = (y_indices * L / self.simulation.H.N) - L/2
        
        scatter_plot = ax.scatter(x_particles, y_particles, c='red', s=10, alpha=0.6, label='Moving Particles')
        ...
        def func_animation(*arg):
            ...
            # Update particle positions (random walk for now)
            #x_particles[:] += np.random.uniform(-0.1, 0.1, num_particles)
            #y_particles[:] += np.random.uniform(-0.1, 0.1, num_particles)
            # Get wavefunction values at particle positions
            x_indices = ((x_particles + L/2) * self.simulation.H.N / L).astype(int)
            y_indices = ((y_particles + L/2) * self.simulation.H.N / L).astype(int)
            x_indices = np.clip(x_indices, 0, self.simulation.H.N-2)
            y_indices = np.clip(y_indices, 0, self.simulation.H.N-2)
            
            psi = self.simulation.Ψ_plot[index][y_indices, x_indices]
            
            # Calculate gradients using finite differences
            psi_dx = self.simulation.Ψ_plot[index][y_indices, x_indices+1] - psi
            psi_dy = self.simulation.Ψ_plot[index][y_indices+1, x_indices] - psi
            
            # Calculate velocities using Bohmian guidance equation
            vx = (hbar/m_e) * np.imag(psi_dx/psi)*2.6# * self.simulation.H.N/L 
            vy = (hbar/m_e) * np.imag(psi_dy/psi)*2.6# * self.simulation.H.N/L
            #print(self.simulation.H.N/L)
            ...
            x_particles[:] = np.clip(x_particles, -L/2, L/2)
            y_particles[:] = np.clip(y_particles, -L/2, L/2)
            scatter_plot.set_offsets(np.c_[x_particles, y_particles])

            return potential_plot, wavefunction_plot, scatter_plot, time_ax

        # Create animation
        a = animation.FuncAnimation(fig, func_animation, blit=True, frames=total_frames, interval=1 / fps * 1000)
